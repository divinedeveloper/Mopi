...
class GiftService {
	...
	def assignGiftToSurvey(survey,gift){
		...
		try
		{
			println "befor update survey for gift survey.status"+survey.status
			survey.gift = gift
			survey.save()
			println "after update survey for gift survey.status "+survey.status
		}
