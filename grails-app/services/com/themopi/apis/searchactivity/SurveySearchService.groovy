...
class SurveySearchService {
	...
	def appSearch(searchCategory,limit,offset,location,authToken,hashTagId){
		...
		def nearestSurveys = []
		def surveyCriteriaTest = Survey.createCriteria()
		double dist = grailsApplication.config.myapp.distanceInKilometerForClosestSearch/100;
		/*def surveysNewList =  surveyCriteriaTest.list(){
			eq('status','Active')
					if(location == "closest"){
						println "Closest"
						println "user.address.location.lat = "+user.address.location.lat
						println "user.address.location.long = "+user.address.location.long
						 
						//double radian = distance/6371;
						
						withinCircle("visiblityCity.location",[[user.address.location.lat,user.address.location.long],dist])
					}
		}
		
		println "Nearest Surveys  == "+surveysNewList*/
		...
		if(searchCategory == "explore" || searchCategory == "hashTag"){
			...
			if(limit.toInteger()+offset.toInteger() <= trendingLimit){
				...
				surveys = surveyCriteria.list(max:limit,offset:offset){
					...
					// closest search not working properly
					if(location == "closest"){
						println "Closest"
						println "user.address.location.lat = "+user.address.location.lat
						println "user.address.location.long = "+user.address.location.long
